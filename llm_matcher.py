...
def _build_prompt(restaurant: RestaurantRow, candidates: Sequence[ScoredCandidate]) -> str:
    ...
    lines = [
        ...
        "- Regional subsidiaries and franchise operating entities are acceptable matches",
        "- Chain stores: a regional operating LLC/Corp is a valid match even if address differs slightly",
        "- National chains (Starbucks, McDonald's, Subway, etc.): do NOT match to a generic holding",
        "  company or licensing entity that covers all PR locations — only match if a candidate clearly",
        "  operates restaurants (not just holds IP or licenses). A pure licensing/IP LLC is NOT a match.",
        "- Parent holding companies with no direct restaurant operations → return null",
        "- If multiple candidates could match, prefer: exact brand name > same city > closest address",
        "- Only return a match if you are confident the candidate actually OPERATES this specific business",
        "",
        "Candidates:",
    ]
